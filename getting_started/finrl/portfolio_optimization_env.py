"""Hacked and patched together version of PortfolioOptimizationEnv.

- The original code was unmaintained, several bug fixes were needed
...
class PortfolioOptimizationEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        if self._terminal:
            ...
            if np.isnan(portfolio_return):
                raise RuntimeError(f"Encountered NaN portfolio return at time index {self._time_index}")
    ...
    def _get_state_and_info_from_time_index(self, time_index):
        ...
        for tic in self._tic_list:
            ...
            # print("Ticker", tic, "time index", time_index, "start time", start_time, "end time", end_time, "state is", len(state) if state is not None else 0, "data is ", len(tic_data))
            # display(state)
            # display(tic_data)
            try:
