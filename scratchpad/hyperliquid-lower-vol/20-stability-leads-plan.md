...
| 1, the vol-matched family (NB21) | REJECT | `drop_30` is a spike, not a rule. It has an empty failure set and gains 11.10 pp of CAGR over the anchor (48.99% against 37.90%, Sharpe 2.747 against 2.160, ulcer 1.384% against 1.796%, beta 0.001), and its lower neighbour fails on ulcer and the late period while `drop_40` fails on three constraints. **[26-backtest-drop-decomposition.ipynb](26-backtest-drop-decomposition.ipynb) then settled what was underneath it**: the data-availability half of the mechanism is exactly inert, reproducing the anchor to six decimals; the volatility half is worth +3.07 pp and survives leave-one-vault-out; and 98% of `drop_30`'s edge is a single vault, since masking Realist Capital takes it to 24.12% against a masked anchor of 23.90%. Its advantage is also confined to the late quarter - before April it is identical to the anchor, because it removes no volatile vault at all. | Whether a plateau exists at a finer spacing than 5. The family was run at 5-step spacing only, and NB26's `measured_only` sweep has had no plateau test or pre-registration. |
