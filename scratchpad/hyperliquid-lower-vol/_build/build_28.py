...
Every target carries an explicit missing-reason code. `forward_event_top5` is NaN unless the
window holds at least 8 positive residual events, so it cannot degenerate towards exactly 1.0 on
a vault that moved three times.
"""))
...
"""))
cells.append(code('''difference = paired_measure_difference(bootstrap, "fresh_event_concentration",
...
display(pd.DataFrame(rows).set_index("signal").round(4))
'''))
